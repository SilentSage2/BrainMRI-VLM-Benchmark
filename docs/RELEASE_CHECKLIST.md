...
Before unsealing, the test split had not been read by any model run. Ruff, mypy, and all 89
tests passed in both the development environment and a clean Python 3.12 editable install.
...
comparison. The explicitly authorized one-shot held-out run subsequently completed on all
66 frozen subjects, supporting the narrow modular-versus-small-grounded-VLM claim. The MVP
tag remains withheld until the final reproduction and review gate. Public text must not
claim that question-conditioned grounding improves answers or that a foundation model was
tested.
